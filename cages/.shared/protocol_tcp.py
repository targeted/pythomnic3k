...
if advanced_ssl_options:
    from ssl import SSLContext, PROTOCOL_SSLv23, OP_NO_SSLv2, OP_NO_SSLv3
...
def _wrap_socket(s, *, ssl_ciphers, ssl_protocol, **kwargs):

    if advanced_ssl_options: # Python 3.2+ contexts are used
        ...
        return ssl_context.wrap_socket(s, do_handshake_on_connect = False, **kwargs)

    else: # Python 3.1 fallback behaviour

        if ssl_ciphers is not None or ssl_protocol is not None:
            pmnc.log.warning("SSL protocol version and ciphers can only "
                             "be configured in Python 3.2 and higher")

        return wrap_socket(s, do_handshake_on_connect = False, **kwargs)
...
class SslConnection(TcpConnection):
    ...
    def __init__(self, interface_name: str, socket, handler_factory: callable, request_timeout: float,
                 ...
                 ssl_ciphers: optional(str), ssl_protocol: optional(one_of("SSLv23", "TLSv1", "TLSv1_1", "TLSv1_2")),
...
class TcpInterface: # called such so as not to be confused with "real" Interface's
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 ssl_protocol: optional(one_of("SSLv23", "TLSv1", "TLSv1_1", "TLSv1_2")),
        ...
        try:
            ...
            try:
                ...
                if self._use_ssl: # perform SSL handshake
                    ...
                    self._tcp_socket = self._socket
                    self._socket = _wrap_socket(self._tcp_socket, server_side = False, keyfile = self._ssl_key_cert_file,
                                                certfile = self._ssl_key_cert_file, ca_certs = self._ssl_ca_cert_file,
                                                ssl_ciphers = self._ssl_ciphers, ssl_protocol = self._ssl_protocol,
...
def self_test():
    ...
    def test_interface(ssl_key_cert_file, ssl_ca_cert_file, ssl_ciphers, ssl_protocol):
        ...
        def connect_to(ifc): # blocking connect
            ...
            if ssl_key_cert_file:
                s = _wrap_socket(s, cert_reqs = CERT_REQUIRED, ssl_protocol = ssl_protocol,
                                 ca_certs = ssl_ca_cert_file, ssl_ciphers = ssl_ciphers)
                try:
                    s.do_handshake()
                except:
                    pmnc.log.error(exc_string())
        ...
        def test_loopback_protocols():
            ifc = start_interface(LineEchoParser, ssl_protocol = "SSLv23")
            try:
                ...
                s = socket(AF_INET, SOCK_STREAM)
                s.connect(ifc.listener_address)
                s = _wrap_socket(s, cert_reqs = CERT_OPTIONAL,
                                 ca_certs = ifc._ssl_ca_cert_file,
                                 ssl_ciphers = ssl_ciphers,
                                 ssl_protocol = "SSLv3")
                try:
                    s.do_handshake()
                except:
                    pmnc.log.error(exc_string()) # this is expected
                else:
                    assert False, "shouldn't be able to connect"
                s.close()
        ...
        if ssl_protocol and advanced_ssl_options:
    ...
    test_interface(None, None, None, None) # tcp
    test_interface(os_path.join(__cage_dir__, "ssl_keys", "key_cert.pem"),
                   os_path.join(__cage_dir__, "ssl_keys", "ca_cert.pem"),
                   advanced_ssl_options and "HIGH:!aNULL:!MD5" or None,
                   advanced_ssl_options and "TLSv1" or None) # ssl
